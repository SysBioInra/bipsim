...
bool ReactionFactory::handle (const std::string& line)
{
  ...
  bool creation_succeeded = (create_chemical_reaction (line)
			     || create_complexation (line)
			     || create_binding (line)
			     || create_elongation (line)
			     || create_release (line)
			     || create_base_loading (line));
...
bool ReactionFactory::create_chemical_reaction ( const std::string& line )
{
  ...
  // create and store
  _cell_state.store (new ChemicalReaction (chemicals, stoichiometries, k_1, k_m1));
  return true;
}
...
bool ReactionFactory::create_elongation ( const std::string& line )
{
  ...
  ProcessiveChemical* processive_chemical = _cell_state.find <ProcessiveChemical> (chemical);
  if (processive_chemical == 0) return false; // TODO throw error ?
  BoundChemical* chemical_after_step = _cell_state.find <BoundChemical> (second_chemical);
  if (chemical_after_step == 0) return false; // TODO throw error ?

  // create and store
  _cell_state.store (new Elongation (*processive_chemical, *chemical_after_step, step_size, rate));
  return true;
}
...
bool ReactionFactory::create_binding ( const std::string& line )
{
  ...
  BoundChemical* binding_result_ptr = _cell_state.find <BoundChemical> (binding_result);
