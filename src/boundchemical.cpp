...
void BoundChemical::focus_random_unit (const BindingSiteFamily& family)
{
  // get the list of positions of chemicalts bound to requested family
  UnitFamilyMap::iterator family_it = _family_map.find (&family);
...
{
  UnitFamilyMap::iterator family_it = 
    _family_map.find (&((_focused_unit->binding_site()).family()));
  BoundUnitList& family_units = family_it->second;
...
// ============================
//
int BoundChemical::number_bound_to_family (const BindingSiteFamily& family) const
{
  // get the list of positions of chemicals bound to requested family
  UnitFamilyMap::const_iterator family_it = _family_map.find (&family);

  // we check whether there are indeed some chemicals in the list
  if (family_it != _family_map.end())
    {
      return (family_it->second).size();
    }
...
double BoundChemical::unbinding_rate_contribution (const BindingSiteFamily& family) const
{
  /**
  ...
  // we check whether there are indeed some chemicals in the list
  if (family_it == _family_map.end()) { return 0; }
  else { return (family_it->second).unbinding_rate(); }
}
...
// =================
//
void BoundChemical::add_unit (const BindingSite& binding_site, 
