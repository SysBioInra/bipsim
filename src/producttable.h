...
class ProductTable : public SimulatorInput
{
 public:
   ...
   */
  void add (const ChemicalSequence& parent, int start, int end,
...
inline void ProductTable::add (const ChemicalSequence& parent,
			       int start, int end, ChemicalSequence& product)
{
  _products [&parent][start][end] = &product;
}
...
ProductTable::generate_child_sequence (ChemicalSequence& parent,
				       int start, int end) const
{
  return _transformation_table.transform (parent.sequence (start, end-start+1));
}
