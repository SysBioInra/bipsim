...
PROGRAMS=("biasedwheel_test" "inputdata_test" "randomhandler_test" \
    ...
    "compositiontable_test" "ratetree_test" "boundunitlist_test" \
...
printf "#########################\n" >> $FILE
printf "#\n" >> $FILE
printf '''AM_CPPFLAGS = $(BOOST_CPPFLAGS)\n''' >> $FILE
...
do
    ...
    printf '''%s_LDADD = $(BOOST_UNIT_TEST_FRAMEWORK_LIB) ../src/libsrc.a\n''' $program >> $FILE
    printf "\n" >> $FILE
done
